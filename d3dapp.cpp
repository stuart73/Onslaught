...
CD3DApplication::CD3DApplication()
{
    ...
    m_dwCreationWidth   = DX_SCREEN_WIDTH;
    m_dwCreationHeight  = DX_SCREEN_HEIGHT;
...
HRESULT CD3DApplication::BuildDeviceList()
{
    ...
    TCHAR strDeviceDescs[2][16];
	
	_tcscpy(strDeviceDescs[0],ToTCHAR("HAL"));
	_tcscpy(strDeviceDescs[1],ToTCHAR("REF"));
        ...
        for( UINT iDevice = 0; iDevice < dwNumDeviceTypes; iDevice++ )
        {
            ...
            for( DWORD f=0; f<dwNumFormats; f++ )
            {
                ...
                if( pDevice->d3dCaps.DevCaps&D3DDEVCAPS_HWTRANSFORMANDLIGHT )
                {
                    ...
                    {
                        dwBehavior[f] = D3DCREATE_HARDWARE_VERTEXPROCESSING | fpumode;
                    ...
                    if ( FALSE == bFormatConfirmed[f] )
                    {
                        dwBehavior[f] = D3DCREATE_MIXED_VERTEXPROCESSING | fpumode;
                ...
                if( FALSE == bFormatConfirmed[f] )
                {
                    dwBehavior[f] = D3DCREATE_SOFTWARE_VERTEXPROCESSING | fpumode;
            ...
            {
                if( pDevice->modes[m].Width==DX_SCREEN_WIDTH && pDevice->modes[m].Height==DX_SCREEN_HEIGHT )
                {
...
/*            // No context menus allowed in fullscreen mode
			...
			// *** Addition for the model viewer

			// No right-click menu in the model viewer or cutscene editor
			if ((CLIPARAMS.mModelViewer) || (CLIPARAMS.mCutsceneEditor)) 
				break;
...
HRESULT CD3DApplication::Initialize3DEnvironment()
{
	...
	if( SUCCEEDED(hr) )
    {
        ...
        if( pDeviceInfo->DeviceType == D3DDEVTYPE_HAL )
        {
            ...
            lstrcat( m_strDeviceStats, ToTCHAR(pAdapterInfo->d3dAdapterIdentifier.Description));
        }

		_tcscat(m_strDeviceStats,ToTCHAR("\n"));
                                                    ...
                                                    WPARAM wParam, LPARAM lParam )
{
    ...
    if( bUpdateDlgControls )
    {
        ...
        for( DWORD m=0; m < pDevice->dwNumModes; m++ )
        {
            ...
            TCHAR strMode[80];
            _stprintf( strMode, ToTCHAR("%ld x %ld x %ld"), pDevice->modes[m].Width,
        ...
        for( m=0; m <= 16; m++ )
        {
            ...
            {
                if( m == 0 )
                    lstrcpy( strDesc, ToTCHAR("none") );
                else
                    wsprintf( strDesc, ToTCHAR("%d samples"), m );
...
VOID CD3DApplication::Cleanup3DEnvironment()
{
    ...
    if( m_pd3dDevice )
    {
        ...
        int drc=m_pd3dDevice->Release();		
        int d3rc=m_pD3D->Release();
		char buf[256];
		sprintf(buf,"d3ddev refcount=%d\n",drc);
		TRACE(buf);
		sprintf(buf,"d3d refcount=%d\n",d3rc);
		TRACE(buf);

#ifdef _DEBUG
		if (((drc>0) || (d3rc>0)) && (CLIPARAMS.mDeveloperMode))
		{
			MessageBox(NULL,ToTCHAR("WARNING : D3D object(s) not freed correctly! Examine debug stream for details..."),ToTCHAR("D3D Resource leak"), MB_ICONERROR|MB_OK);
		}
#endif 
...
HRESULT CD3DApplication::DisplayErrorMsg( HRESULT hr, DWORD dwType )
{
    ...
    switch( hr )
    {
        case D3DAPPERR_NODIRECT3D:
            _tcscpy( strMsg, ToTCHAR("Could not initialize Direct3D. You may\n\
        ...
        case D3DAPPERR_NOCOMPATIBLEDEVICES:
            _tcscpy( strMsg, ToTCHAR("Could not find any compatible Direct3D\ndevices.") );
            break;

        case D3DAPPERR_NOWINDOWABLEDEVICES:
            _tcscpy( strMsg, ToTCHAR("This sample cannot run in a desktop\n\
        ...
        case D3DAPPERR_NOHARDWAREDEVICE:
            _tcscpy( strMsg, ToTCHAR("No hardware-accelerated Direct3D devices\n\
                             were found.") );
            break;

        case D3DAPPERR_HALNOTCOMPATIBLE:
            _tcscpy( strMsg, ToTCHAR("This sample requires functionality that is\n\
                             not available on your Direct3D hardware\n\
                             accelerator.") );
            break;

        case D3DAPPERR_NOWINDOWEDHAL:
            _tcscpy( strMsg, ToTCHAR("Your Direct3D hardware accelerator cannot\n\
        ...
        case D3DAPPERR_MEDIANOTFOUND:
            _tcscpy( strMsg, ToTCHAR("Could not load required media." ) );
            break;

        case D3DAPPERR_RESIZEFAILED:
            _tcscpy( strMsg, ToTCHAR("Could not reset the Direct3D device." ) );
            break;

        case D3DAPPERR_NONZEROREFCOUNT:
            _tcscpy( strMsg, ToTCHAR("A D3D object has a non-zero reference\n\
                             count (meaning things were not properly\n\
                             cleaned up).") );
            break;

        case E_OUTOFMEMORY:
            _tcscpy( strMsg, ToTCHAR("Not enough memory.") );
            break;

        case D3DERR_OUTOFVIDEOMEMORY:
            _tcscpy( strMsg, ToTCHAR("Not enough video memory.") );
            break;

        default:
            _tcscpy( strMsg, ToTCHAR("Generic application error. Enable\n\
                             debug output for detailed information.") );
    }

    if( MSGERR_APPMUSTEXIT == dwType )
    {
        _tcscat( strMsg, ToTCHAR("\n\nThis sample will now exit.") );
